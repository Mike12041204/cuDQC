...
void CPU_Graph::GenLevel2NBs()
{
	...
	memset(temp_bool_array1, 0, number_of_vertices * sizeof(int));
	...
	for(int i = 0; i < number_of_vertices; i++){
		...
		for(uint64_t j = 0; j < in_size; j++){
			...
			if(temp_int_array1[vertexid1] == 1){
				temp_int_vector5.push_back(vertexid1);
			}
			else{
		...
		do{
			...
			for(uint64_t j = 0; j < size; j++){
				...
				for(uint64_t k = 0; k < size; k++){
					...
					if(temp_int_array4[vertexid1] == round){
						temp_int_array4[vertexid1]++;
						temp_int_vector4.push_back(vertexid1);
					}
				}
			}
			...
			for(uint64_t j = 0; j < size; j++){
				...
				for(uint64_t k = 0; k < size; k++){
					...
					if(temp_int_array3[vertexid1] == round){
						temp_int_array3[vertexid1]++;
						temp_int_vector3.push_back(vertexid1);
					}
				}
			}
		...
		// at bi-neighbors, O, and I twohop adj into results
		size = temp_int_vector5.size();
		for(uint64_t j = 0; j < size; j++){
			vertexid1 = temp_int_vector5.at(j);
		...
		for(uint64_t j = 0; j < size; j++){
			...
			vertexid1 = temp_int_vector1.at(j);
			size2 = out_offsets[j + 1] - out_offsets[j];
			start_write = out_offsets[j];
		...
		for(uint64_t j = 0; j < size; j++){
			...
			vertexid1 = temp_int_vector1.at(j);
			size2 = in_offsets[j + 1] - in_offsets[j];
			start_write = in_offsets[j];
		...
		// insert into graph twohop structure
