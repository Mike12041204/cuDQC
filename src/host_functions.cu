                              ...
                              int minimum_clique_size)
{
    ...
    chkerr(cudaMalloc((void**)&h_dd.temp_int_array_2, (sizeof(int) * dss.WVERTICES_SIZE) * NUMBER_OF_WARPS));
    chkerr(cudaMalloc((void**)&h_dd.temp_int_array_3, (sizeof(int) * dss.WVERTICES_SIZE) * NUMBER_OF_WARPS));
    chkerr(cudaMalloc((void**)&h_dd.temp_int_array_4, (sizeof(int) * dss.WVERTICES_SIZE) * NUMBER_OF_WARPS));
    chkerr(cudaMalloc((void**)&h_dd.temp_int_array_5, (sizeof(int) * dss.WVERTICES_SIZE) * NUMBER_OF_WARPS));
...
void h_condense_graph(CPU_Data& hd, CPU_Graph& hg, Vertex* vertices, int number_of_candidates)
{
    ...
    results = new vector<int>[number_of_candidates];
    ...
    hg.original_id_map = new int[number_of_candidates];

    // initialize vertexid to index maps
    for(int i = 0; i < number_of_candidates; i++){
        pvertexid = vertices[i].vertexid;
        hg.original_id_map[i] = pvertexid;
        vertexid_to_index[pvertexid] = i;
    }
    ...
    for(int i = 0; i < number_of_candidates; i++){
        ...
        for(uint64_t j = pneighbors_start; j < pneighbors_end; j++){
            phelper1 = hg.twohop_neighbors[j];
            phelper2 = hd.vertex_order_map[phelper1];

            if(phelper2 > -1){
                results[i].push_back(vertexid_to_index[phelper1]);
            }
        }

        sort(results[i].begin(), results[i].end());
        number_of_lvl2adj += results[i].size();

        // we calculate twohop adj as biproduct of condensing graph, used for lookahead pruning
        vertices[i].lvl2adj = results[i].size();
    }

    // rewrite twohop adj
    for(int i = 0; i < number_of_candidates; i++){
        hg.twohop_offsets[i + 1] = hg.twohop_offsets[i] + results[i].size();

		pneighbors_start = hg.twohop_offsets[i];
        pneighbors_end = results[i].size();
		for(uint64_t j = 0; j < pneighbors_end; j++){
			hg.twohop_neighbors[pneighbors_start + j] = results[i].at(j);
		}
    ...
    for(int i = 0; i < number_of_candidates; i++){
        ...
        sort(results[i].begin(), results[i].end());
        number_of_edges += results[i].size();
    }

    // rewrite out adj
    for(int i = 0; i < number_of_candidates; i++){
        hg.out_offsets[i + 1] = hg.out_offsets[i] + results[i].size();

		pneighbors_start = hg.out_offsets[i];
        pneighbors_end = results[i].size();
		for(uint64_t j = 0; j < pneighbors_end; j++){
			hg.out_neighbors[pneighbors_start + j] = results[i].at(j);
		}
    ...
    // rewrite in adj
    for(int i = 0; i < number_of_candidates; i++){
        hg.in_offsets[i + 1] = hg.in_offsets[i] + results[i].size();

		pneighbors_start = hg.in_offsets[i];
        pneighbors_end = results[i].size();
		for(uint64_t j = 0; j < pneighbors_end; j++){
			hg.in_neighbors[pneighbors_start + j] = results[i].at(j);
		}
...
{       
    ...
    *tasks_count = 0;
    offset_start = 0;
    for(uint64_t i = grank; i < count; i += wsize){
        // increment assigned tasks count
        (*tasks_count)++;

        // copy vertices before offets are changed
        for(uint64_t j = tasks_offset[i]; j < tasks_offset[i + 1]; j++){
    ...
    *hd.buffer_count = 0;
    ...
    for(uint64_t i = grank; i < count; i += wsize){
        ...
        // copy vertices before offets are changed
        for(uint64_t j = hd.buffer_offset[i]; j < hd.buffer_offset[i + 1]; j++){
...
void h_free_memory(CPU_Data& hd, GPU_Data& h_dd, CPU_Cliques& hc)
{
    ...
    chkerr(cudaFree(h_dd.temp_int_array_2));
    chkerr(cudaFree(h_dd.temp_int_array_3));
    chkerr(cudaFree(h_dd.temp_int_array_4));
    chkerr(cudaFree(h_dd.temp_int_array_5));
                               ...
                               int minimum_clique_size, int& success)
{
    ...
    adjacencies = new int[total_vertices];
    memset(adjacencies, 0, sizeof(int) * total_vertices);
    ...
    for (int i = 0; i < number_of_members; i++)
    {
        ...
        if (vertices[i].out_mem_deg + vertices[i].out_can_deg == 
            minimum_out_degrees[number_of_members + lower_bound] && vertices[i].out_can_deg > 0) {
        ...
        // if they are a critical vertex
        if (vertices[i].in_mem_deg + vertices[i].in_can_deg == 
            minimum_in_degrees[number_of_members + lower_bound] && vertices[i].in_can_deg > 0) {
    ...
    // calculate adjacencies, adjacencies to critical vertices
    for (int i = number_of_members; i < number_of_members + number_of_crit; i++) {
    ...
    for (int i = number_of_members; i < number_of_members + number_of_crit; i++) {
        if (adjacencies[i] < number_of_crit - 1) {
            success = 2;
            break;
        }
    }
    ...
    // update adjacencies of critical adjacent vertices
    for (int i = number_of_members; i < number_of_members + number_of_crit; i++) {
    ...
    number_of_members += number_of_crit;
    number_of_candidates -= number_of_crit;

    // DIAMTER PRUNING
    h_diameter_pruning_cv(hd, vertices, total_vertices, number_of_members, adjacencies, 
                          number_of_crit);
...
void h_diameter_pruning_cv(CPU_Data& hd, Vertex* vertices, int& total_vertices, 
                           int number_of_members, int* adjacencies, int number_of_crit)
{
    *hd.remaining_count = 0;
    ...
    for (int i = number_of_members; i < total_vertices; i++) {
        ...
        if (adjacencies[i] == number_of_crit) {
            ...
            hd.candidate_out_mem_degs[*hd.remaining_count] = vertices[i].out_mem_deg;
            hd.candidate_in_mem_degs[*hd.remaining_count] = vertices[i].in_mem_deg;
            (*hd.remaining_count)++;
        }
        else {
            vertices[i].label = -1;
                      ...
                      int minimum_clique_size, int& success)
{
    ...
    for (int i = 0; i < number_of_members; i++) {
        if (!h_vert_isextendable(vertices[i], number_of_members, upper_bound, lower_bound, 
    ...
    while (*hd.remaining_count > 0 && *hd.removed_count > 0) { 
        ...
        for (int i = 0; i < (*hd.remaining_count); i++) {
            if (h_cand_isvalid(vertices[hd.remaining_candidates[i]], number_of_members, 
                               upper_bound, lower_bound, min_ext_out_deg, min_ext_in_deg, minimum_out_degrees, 
                               minimum_in_degrees, minimum_clique_size)) {

                hd.candidate_out_mem_degs[num_val_cands] = 
                    vertices[hd.remaining_candidates[i]].out_mem_deg;
                hd.candidate_in_mem_degs[num_val_cands] = 
                    vertices[hd.remaining_candidates[i]].in_mem_deg;
                num_val_cands++;
            }
        }
        ...
        for (int i = 0; i < (*hd.remaining_count); i++) {
            if (h_cand_isvalid(vertices[hd.remaining_candidates[i]], number_of_members, 
                ...
                hd.remaining_candidates[num_val_cands++] = hd.remaining_candidates[i];
            }
            else {
                hd.removed_candidates[(*hd.removed_count)++] = hd.remaining_candidates[i];
            }
        }
                        ...
                        int* minimum_out_degrees, int* minimum_in_degrees, int minimum_clique_size)
{
    ...
    for (int i = 0; i < number_of_members; i++) {
        if (vertices[i].out_mem_deg < min_out_deg || vertices[i].in_mem_deg < min_in_deg) {
            clique = false;
            break;
        }
    }
